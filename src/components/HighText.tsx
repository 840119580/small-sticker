...
import { HTMLAttributes, useEffect, useState, forwardRef, FormEvent } from "react";
...
const HighText = forwardRef<HTMLDivElement, {
  ...
  // 分离外部传入的样式属性
  const { style: externalStyle, className: externalClassName, ...otherAttrs } = attrs;
  ...
  const handleInput = (e: FormEvent<HTMLDivElement>) => {
    ...
    const cursorPosition = range ? range.startOffset : div.textContent?.length || 0;
    ...
    >
      ...
      {show ?
        <div className={`${large ? 'text-4xl font-500 mt-1' : 'text-xs'}`}>
          ...
          {eg ?
            <div 
              style={{ color: '#6B7280', display: 'flex', alignItems: 'center', paddingBottom: '0.125rem', marginTop: '-0.125rem' }} 
